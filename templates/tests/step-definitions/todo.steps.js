...
Then('I should see "Email: test@example.com"', async function() {
    ...
    if(!email){
        console.log(chalk.red('Email is not set in the environment variables, Please try "')+
            chalk.yellow('bingo mask email')+ chalk.blue('YOUR_EMAIL')+ chalk.red('" to set the email'));
...
Then('I should see "Password: secret"', async function() {
    ...
    if(!password){
        console.log(chalk.red('Password is not set in the environment variables, Please try "')+
            chalk.yellow('bingo mask password')+ chalk.blue('YOUR_PASSWORD')+ chalk.red('" to set the password'));
            console.log("After masking use it place it in the env file")
            console.log("And to use this just do " + chalk.green('env.TEST_PASSWORD'));
    }
    if (password !== 'secret') {
