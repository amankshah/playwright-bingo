...
class BingoPage {
    constructor(page) {
        this.page = page;
        this.config = config;
        // Return a Proxy to forward unknown properties/methods to the underlying page
        return new Proxy(this, {
            get(target, prop, receiver) {
                // If the property exists on BingoPage, use it
                if (prop in target) {
                    return Reflect.get(target, prop, receiver);
                }
                // Otherwise, forward to the underlying Playwright page
                const value = target.page[prop];
                // If it's a function, bind it to the page
                if (typeof value === 'function') {
                    return value.bind(target.page);
                }
                return value;
            }
        });
